...
import os, shutil, json, platform, subprocess, threading
...
class BookmarkPanel(tk.Frame):
    ...
    def _build(self):
        ...
        hdr = tk.Frame(self, bg=p['toolbar_bg'], pady=4)
        hdr.pack(fill='x')
        tk.Label(hdr, text='⭐ 즐겨찾기', font=(UI_FONT, 9, 'bold'),
                 bg=p['toolbar_bg'], fg=p['toolbar_fg']).pack(side='left', padx=8)
        tk.Button(hdr, text='+', command=self._add_current,
                  bg=p['accent'], fg=p['accent_fg'], relief='flat',
                  font=(UI_FONT, 9, 'bold'), width=2, cursor='hand2').pack(side='right', padx=4)

        sb = ttk.Scrollbar(self)
        sb.pack(side='right', fill='y')
        self._lb = tk.Listbox(self, yscrollcommand=sb.set,
        ...
        self._lb.bind('<Double-Button-1>', self._navigate)
        self._lb.bind('<Button-3>',        self._context_menu)
        self._refresh_lb()

        # 현재 경로 참조용 (외부에서 set 호출)
        self.current_path_ref = None

    def _refresh_lb(self):
        self._lb.delete(0, 'end')
        for bm in self._bookmarks:
            icon = '📂' if os.path.isdir(bm) else '📄'
            self._lb.insert('end', f'{icon}  {os.path.basename(bm) or bm}')

    def _navigate(self, event=None):
        sel = self._lb.curselection()
        if sel and self._on_navigate:
            self._on_navigate(self._bookmarks[sel[0]])

    def _context_menu(self, event):
        sel = self._lb.nearest(event.y)
        if sel < 0 or sel >= len(self._bookmarks):
            return
        ...
        menu.add_command(label='이동',   command=self._navigate)
        menu.add_command(label='삭제',   command=lambda: self._remove(sel))
...
class FilePanel(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(filter_row, text='필터', bg=p['toolbar_bg'],
                 fg=p['toolbar_fg'], font=(UI_FONT, 8)).pack(side='left', padx=(0, 6))
...
class SimpleCommander:
    def __init__(self, root: tk.Tk):
        self.root     = root
        self.root.title('Simple Commander v2.0')
    ...
    def _setup_shortcuts(self):
        binds = [
            ...
            ('<Control-f>',    lambda _: (self._search_entry.focus_set(), None)),
    ...
    def _show_search_results(self, query: str, results: list, base_path: str):
        ...
        dlg.title(f'검색 결과: "{query}"  ({len(results)}개)')
        ...
        hdr.pack(fill='x')
        tk.Label(hdr, text=f'🔍  "{query}" 검색 결과  |  {len(results)}개',
                 bg=p['toolbar_bg'], fg=p['toolbar_fg'],
                 font=(UI_FONT, 10, 'bold')).pack(side='left', padx=12)
        ...
        tk.Button(bf, text='열기', command=on_open,
                  bg=p['accent'], fg=p['accent_fg'], relief='flat',
